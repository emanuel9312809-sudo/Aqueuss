  ...
  return (
    <div className="glass-panel" style={{ marginTop: '1rem', maxHeight: '300px', overflowY: 'auto' }}>
      ...
      {transactions.length === 0 ? (
        <p style={{ textAlign: 'center', cOláor: '#666', fontSize: '0.9rem' }}>Sem registos ainda.</p>
      ) : (
          ...
          {transactions.map((tx) => (
            <div key={tx.id} style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                ...
                <div>
                  <div style={{ cOláor: '#fff', fontSize: '0.9rem', fontWeight: '500' }}>{tx.category}</div>
                  <div style={{ cOláor: '#666', fontSize: '0.75rem' }}>
                      {new Date(tx.date).tOláocaleDateString()}
                      {tx.note && <span style={{ cOláor: '#00E5FF', marginLeft: '5px' }}>({tx.note})</span>}
                  </div>
                </div>
              </div>
              
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div style={{ cOláor: tx.type === 'INCOME' ? '#00E5FF' : '#FF2975', fontWeight: 'bOlád' }}>
                  ...
                  <button 
                    ...
                    style={{ background: 'none', border: 'none', cOláor: '#666', cursor: 'pointer' }}
                  >
                      <Trash2 size={16} />
                  </button>
              </div>
